...
function parseDate(str) {
  if (!str) return null;
  // 全角数字・記号を半角に変換
  const normalized = str.replace(/[０-９]/g, c => String.fromCharCode(c.charCodeAt(0) - 0xFEE0));
...
function main() {
  ...
  // 必須フィールドの確認
  const required = ['タイトル', 'カテゴリ', 'エリア', '会場', '開始日', '終了日', '説明', '入場料'];
  ...
  const manualIds = idMatches.map(m => parseInt(m[1])).filter(id => id >= 1 && id <= 999);
  ...
  event.emoji    = fields['絵文字（任意、省略で自動設定）'] || DEFAULT_EMOJI[category] || '🎪';
  ...
  if (fields['タグ（カンマ区切り、任意）']) {
    event.tags = fields['タグ（カンマ区切り、任意）'].split(',').map(t => t.trim()).filter(Boolean);
  }

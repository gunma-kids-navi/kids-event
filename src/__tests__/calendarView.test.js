...
// ========================================================
// 市町村フィルター（areaOptions / displayEvents のエリア絞り込み）
// ========================================================

// CalendarView の areaOptions ロジックを再現
...
function getAreaOptions(events, year, month, selectedDay) {
  const base = selectedDay
    ? getEventsOnDay(events, year, month, selectedDay)
    : getMonthEvents(events, year, month);
...
// CalendarView の displayEvents（エリアフィルター付き）を再現
function getDisplayEventsWithArea(events, year, month, selectedDay, filterArea) {
  const base = selectedDay
    ? getEventsOnDay(events, year, month, selectedDay)
    : getMonthEvents(events, year, month);
  if (filterArea === "all") return base;
  return base.filter((ev) => ev.area === filterArea);
}

describe("CalendarView 市町村フィルター areaOptions", () => {
  it("月全体（selectedDay=null）のエリア一覧を返す", () => {
    // 5月: id:1=前橋市, id:2=高崎市, id:4=太田市
    const result = getAreaOptions(MOCK_EVENTS, 2026, 4, null);
    expect(result).toContain("前橋市");
    expect(result).toContain("高崎市");
    expect(result).toContain("太田市");
    expect(result).not.toContain("桐生市"); // 6月のイベントなので含まれない
  });

  it("selectedDay が設定された場合はその日のエリアのみ返す", () => {
    // 5月5日: id:2=高崎市, id:4=太田市
    const result = getAreaOptions(MOCK_EVENTS, 2026, 4, 5);
    expect(result).toContain("高崎市");
    expect(result).toContain("太田市");
    expect(result).not.toContain("前橋市");
  });

  it("エリアは重複なしで返す", () => {
    const multiAreaEvents = [
      { id: 10, title: "A", startDate: "2026-05-01", endDate: "2026-05-10", area: "前橋市", emoji: "🎈", category: "experience", categoryLabel: "体験", venue: "V", tags: [], desc: "", url: "", free: true, age: "全" },
      { id: 11, title: "B", startDate: "2026-05-05", endDate: "2026-05-15", area: "前橋市", emoji: "🎪", category: "festival", categoryLabel: "祭り", venue: "V", tags: [], desc: "", url: "", free: false, age: "全" },
    ];
    const result = getAreaOptions(multiAreaEvents, 2026, 4, null);
    expect(result.filter((a) => a === "前橋市").length).toBe(1);
  });

  it("市町村コード順（前橋市 < 高崎市 < 太田市）にソートされる", () => {
    const result = getAreaOptions(MOCK_EVENTS, 2026, 4, null);
    const idxMaebashi = result.indexOf("前橋市");
    const idxTakasaki = result.indexOf("高崎市");
    const idxOta = result.indexOf("太田市");
    expect(idxMaebashi).toBeLessThan(idxTakasaki);
    expect(idxTakasaki).toBeLessThan(idxOta);
  });

  it("エリアが1種類しかない月はリスト長が1", () => {
    const singleAreaEvents = [
      { id: 20, title: "X", startDate: "2026-05-01", endDate: "2026-05-31", area: "沼田市", emoji: "🌟", category: "nature", categoryLabel: "自然", venue: "V", tags: [], desc: "", url: "", free: false, age: "全" },
    ];
    const result = getAreaOptions(singleAreaEvents, 2026, 4, null);
    expect(result.length).toBe(1);
    expect(result[0]).toBe("沼田市");
  });
});

describe("CalendarView displayEvents エリア絞り込み", () => {
  it("filterArea='all' のとき絞り込みなしで全件返す", () => {
    const result = getDisplayEventsWithArea(MOCK_EVENTS, 2026, 4, null, "all");
    expect(result.length).toBe(3); // 5月: id:1,2,4
  });

  it("filterArea='前橋市' のとき前橋市のイベントのみ返す", () => {
    const result = getDisplayEventsWithArea(MOCK_EVENTS, 2026, 4, null, "前橋市");
    expect(result.every((ev) => ev.area === "前橋市")).toBe(true);
    expect(result.some((ev) => ev.area === "高崎市")).toBe(false);
  });

  it("selectedDay + filterArea の組み合わせで正しく絞り込める", () => {
    // 5月5日: id:2=高崎市, id:4=太田市
    const result = getDisplayEventsWithArea(MOCK_EVENTS, 2026, 4, 5, "高崎市");
    expect(result.length).toBe(1);
    expect(result[0].id).toBe(2);
  });

  it("該当エリアのイベントがない場合は空配列を返す", () => {
    const result = getDisplayEventsWithArea(MOCK_EVENTS, 2026, 4, null, "桐生市");
    expect(result.length).toBe(0);
  });

  it("月変更後は filterArea='all' にリセットされる（仕様確認）", () => {
    let filterArea = "前橋市";
    // prevMonth/nextMonth 時にリセットする仕様
    const simulateMonthChange = () => { filterArea = "all"; };
    simulateMonthChange();
    expect(filterArea).toBe("all");
  });

  it("日付変更後は filterArea='all' にリセットされる（仕様確認）", () => {
    let filterArea = "高崎市";
    const simulateToggleDay = () => { filterArea = "all"; };
    simulateToggleDay();
    expect(filterArea).toBe("all");
  });
});
